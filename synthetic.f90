...
module synthetic
   ...
   ! ============================================================
   logical, parameter :: write_sed_per_model = .true.
   
   ! Internal counter for model tracking (used when write_sed_per_model = .true.)
   integer, save :: sed_model_counter = 0
...
contains
   ...
   real(dp) function calculate_synthetic(temperature, gravity, metallicity, ierr, &
      ...
      if (make_sed) then
         ...
         if (write_sed_per_model) then
            ! First call: record the first filter name
            if (first_filter_name == '') first_filter_name = trim(filter_name)
            ! Increment counter when we see the first filter again (new model)
            if (trim(filter_name) == trim(first_filter_name)) sed_model_counter = sed_model_counter + 1
            
            write(model_suffix, '(A,I0)') '_', sed_model_counter
            csv_file = trim(colors_results_directory)//'/'//trim(remove_dat(filter_name))//'_SED'//trim(model_suffix)//'.csv'
         else
